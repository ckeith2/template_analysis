...
def get_ktests(save= False):
    ...
    if save:
        ...
        hdulist.writeto('ktestsfile1MeVfin5yrs3.fits')
    return ktest_array
...
def get_likelihoodat(fbh, name_of_file = 'testingnew/', dmfilename = 'yield_DS_keith40.dat'):
    energies = np.copy(central_energies)
    deltae_cut = np.copy(deltae)
    ...
    for energyidx in range(0, 13):
        ...
        #Dark matter
        darkmtest = poisson_dist(np.nan, int(energyidx), dm = True, cross_section = test_cross, analyze_data = False, filenm = dmfilename)
        darkmtest[np.isnan(darkmtest)] = 0
...
def twosig_fbh(testfbh, likefbh0):
    print(10**testfbh)
    return root_find(float(10**testfbh))-4-likefbh0
...
filelist = ['bremss_healpix_reshuffled_42templates_0511MeV.fits', 'bremss_healpix_reshuffled_42templates_0511MeV.fits', 'pi0_decay_healpix_reshuffled_42templates_0511MeV.fits', 'pi0_decay_healpix_reshuffled_42templates_0511MeV.fits', 'ics_isotropic_healpix_reshuffled_42templates_0511MeV.fits', 'ics_isotropic_healpix_reshuffled_42templates_0511MeV.fits']
...
point_sources = ['PS_nonfloat_511MeV_0-41.fits']
...
bremss_reopen = fits.open('bremss_healpix_reshuffled_42templates_0511MeV.fits')
...
ktest_array = readfile('ktestsfile1MeVfin5yrs2.fits')[0].data
